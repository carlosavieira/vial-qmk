...
void rgb_matrix_set_layer_color(int index, int layer) {
    rgb_matrix_set_hsv_color(index, hsv_hue_shift(rgb_matrix_config.hsv, layer * RGB_MATRIX_LAYER_HUE_DELTA));
}

int resolve_effective_kc_layer(keypos_t keypos, int layer) {
    if (layer == 0) return 0;
    if (IS_LAYER_OFF(layer)) return resolve_effective_kc_layer(keypos, layer - 1);
        ...
        for (uint8_t col = 0; col < MATRIX_COLS; ++col) {
            ...
            if (last_input_activity_elapsed() > (uint32_t)RGB_MATRIX_IDLE_TIMEOUT) {
                ...
                if (layer == default_layer) {
                    // capslock
                    if (row == rgb_matrix_capslock_indicator_pos[0] && col == rgb_matrix_capslock_indicator_pos[1]) {
                    ...
                    if (keymap_key_to_keycode(layer, keypos) == KC_NO) {
                        ...
                        uint8_t kc_layer = resolve_effective_kc_layer(keypos, layer);

                        if (kc_layer > 0) {
                            rgb_matrix_set_layer_color(index, kc_layer);
                        }
                    }
                }
            }
